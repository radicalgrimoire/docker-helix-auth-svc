...
ACCESS_TOKEN=$(curl --fail -H "Metadata: true" "http://169.254.169.254/metadata/identity/oauth2/token?resource=https://vault.azure.net&api-version=2018-02-01" | jq -r '.access_token')

SECRET_URL="https://${KEYVAULT_NAME}.vault.azure.net/secrets/${CERTIFICATE_NAME}/?api-version=2016-10-01"

curl --fail -H "Authorization: Bearer $ACCESS_TOKEN" "$SECRET_URL" | jq -r '.value' | base64 -id > "${TMP_DUMP_PFX}"
